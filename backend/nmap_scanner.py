...
def create_nmap_scanner_tab(notebook):
    ...
    make_subtitle(
        nmap_tab,
        "Advanced scan using Nmap. Only scan systems you own or have permission to test."
    )
    ...
    def run_nmap_scan():
        ...
        if not target:
            messagebox.showwarning(
                "Input Error",
                "Please enter a target like 127.0.0.1 or example.com."
            )
            return
        ...
        try:
            ...
            result_text.delete("1.0", tk.END)
            result_text.insert(tk.END, final_output)
            ...
            result_text.insert(
                tk.END,
                "Nmap scan timed out. Try a smaller target or simpler scan.\n"
            )
    ...
    def export_nmap_pdf():
        ...
        write_pdf("Nmap Scanner Report", content)
